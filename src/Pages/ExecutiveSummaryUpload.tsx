...
const ExecutiveSummaryUpload: React.FC = () => {
  ...
  const fetchUploadHistory = useCallback(async () => {
    ...
    try {
      // Single API call — returns { months: ['MM/YYYY', ...] }  e.g. ['01/2026','12/2025']
      ...
      const rows: UploadRecord[] = uploadedMonths.map((raw) => {
  ...
  const handleFinancialUpload = async (file: File, isReplace: boolean = false) => {
    ...
    try {
      ...
      await blobService.uploadExcelToBlob("ExecutiveSummary", dateStr, base64);
  ...
  const handleDownload = async (rec: UploadRecord) => {
    try {
      ...
      await blobService.downloadFileFromBlob("ExecutiveSummary", dateStr);
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <Box>
          ...
          <Typography sx={{ fontSize: '13px', color: '#64748B', mt: 0.5 }}>
            Upload monthly CAPEX files and manage previously submitted reports.
          </Typography>
        </Box>
